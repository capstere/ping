(() => {
  ...
  hintBtn?.addEventListener("click", ()=>{
    ...
    if (state.unlocked){
      bubble("Redan klart. 🚪 Hemlig sida om du måste.", 1700);
      return;
    }
    if (state.secretStep === 0){
      bubble("Börja med ⏰. Punkt.", 1700);
    } else if (state.secretStep === 1){
      bubble("🍬. Gör det bara.", 1700);
...
setSoundBtnLabel();
updateAchUI();
if (audioEnabled) setTimeout(()=>toast("Ljud: på. (Tryck igen om Safari surar.)", 2200), 650);
...
if (!("PointerEvent" in window)){
  ...
  function advanceSecret(id){
    ...
    if (Math.random() < 0.25){
      const lines = ["Japp.", "Helt avsiktligt.", "Jag är imponerad. Lite.", "Klart."];
  ...
  function doDance(){
    ...
    bubble("🕺 Dans. (ingen filmning).", 1600);
  }
