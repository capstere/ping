...
function Add-OpenXmlDebug {
    param([string]$Message)
    try {
        if (-not $script:OpenXmlDebugLog) { $script:OpenXmlDebugLog = New-Object System.Collections.Generic.List[string] }
        $script:OpenXmlDebugLog.Add(($Message + '')) | Out-Null
    } catch {}
}

function Get-OpenXmlDebugLog {
    try { return $script:OpenXmlDebugLog } catch { return @() }
}
    ...
    if (-not $Overwrite) {
        if ($mergeRefs -and $mergeRefs.Count -gt 0) {
            foreach ($rref in $mergeRefs) {
                ...
                if ($t) { Add-OpenXmlDebug ("SKIP existing in merge tail: ref=$rref value='$t' (target=$origTargetRef owner=$ownerRef)"); return $false }
            }
        }
        if ($existing) { Add-OpenXmlDebug ("SKIP existing in owner: ref=$ownerRef value='$existing' (target=$origTargetRef)"); return $false }
    } else {
...
function Invoke-WorksheetSignature_OpenXml {
    ...
    # Deterministic bool for overwrite (avoid SwitchParameter edge cases)
    $overwriteBool = $false
    ...
    try {
        ...
        foreach ($t in $targets) {
            ...
            $wroteAny = $false
            if ($nameRow) {
                $nameWritten = (Set-OpenXmlCellText -WorksheetPart $wsp -WorkbookPart $wbp -RowIndex ($nameRow + $t.Offset) -ColLetter $t.NameWriteCol -Value $FullName -Overwrite $overwriteBool -MergeMap $mergeMap -MergeRanges $mergeRanges -TailFillValue 'N/A')
                if ($nameWritten) {
            ...
            if ($dateRow) {
                $dateWritten = (Set-OpenXmlCellText -WorksheetPart $wsp -WorkbookPart $wbp -RowIndex ($dateRow + $t.Offset) -ColLetter $t.DateWriteCol -Value $SignDateYmd -Overwrite $overwriteBool -MergeMap $mergeMap -MergeRanges $mergeRanges -TailFillValue 'N/A')
                if ($dateWritten) {
