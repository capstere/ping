...
function Get-UpdateLogSchema {
    param([Parameter(Mandatory=$true)][string]$Path)

    if (-not (Test-Path -LiteralPath $Path)) { return "v2" } # ny fil => ny header
    try {
        $first = Get-Content -LiteralPath $Path -TotalCount 1 -ErrorAction Stop
        if ($first -match ';\s*Action\s*;' -or $first -match '^Timestamp;User;Signature;Action;') { return "v2" }
        return "v1"
    } catch {
        # Om vi inte kan läsa header av någon anledning, anta v1 (minst risk att "spräcka" format)
        return "v1"
    }
}
...
function Ensure-UpdateLogCsv {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        try {
            # UTF8 *med BOM* => Excel på Windows öppnar korrekt + minskar risken för "tomma" rader pga encoding-strul.
...
function Get-UpdateLogSpoolPath {
    ...
    $spoolName = "{0}_SPOOL_{1}.csv" -f $name, $env:COMPUTERNAME
    return (Join-Path $env:TEMP $spoolName)
}
...
function Write-UpdateLogSpoolLine {
    ...
    # Spool är lokal => mycket hög sannolikhet att det alltid fungerar.
    if (-not (Test-Path -LiteralPath $spool)) {
        # Skriv samma header som main (v2) för enkel merge.
        $header = 'Timestamp;User;Signature;Action;PN;Row;OldLot;OldExp;OldQty;NewLot;NewExp;NewQty;Machine'
        $utf8Bom = New-Object System.Text.UTF8Encoding($true)
        [System.IO.File]::WriteAllText($spool, $header + [Environment]::NewLine, $utf8Bom)
    }
    # Append utan fancy lock behövs ej lokalt.
    Add-Content -LiteralPath $spool -Value $Line -Encoding UTF8
}
...
function Flush-UpdateLogSpool {
    ...
    # Läs alla rader utom header och försök appenda dem till main.
    try {
        $lines = Get-Content -LiteralPath $spool -ErrorAction Stop
        if ($lines.Count -le 1) {
            Remove-Item -LiteralPath $spool -Force -ErrorAction SilentlyContinue
            return
        }
...
function Write-UpdateLogRow {
    ...
    $schema = Get-UpdateLogSchema -Path $path
    ...
    if ($schema -eq "v2") {
        ...
        # Bakåtkompatibel: gammal header utan Action/Row
        $line = (@(
            CsvCell $ts,
            CsvCell $user,
            CsvCell $Signature,
    ...
    try {
        ...
        throw "CSV-loggen var låst/otillgänglig. Loggrad skrevs till lokal spool i %TEMP% och kommer flushas automatiskt nästa gång. Orsak: $($_.Exception.Message)"
    }
}
